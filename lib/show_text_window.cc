...
show_text_window::show_text_window(int splitlength, int maxlines, QWidget* parent)
    : QWidget(parent)
{
    /** Build gui */
    // Header text
    ...
    connect(d_clear, SIGNAL(clicked()), this, SLOT(clearText()));
...
void show_text_window::autofocus_enable(bool b) { d_autofocus = b; }

void show_text_window::insert_nl_into_line(int from)
{
...
void show_text_window::move_input2lines()
{
...
void show_text_window::scroll_up(unsigned int rows, bool clear)
{
    if (rows <= 0)
        return;
